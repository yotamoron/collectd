...
This output plugin submits values to a MySQL server. You need first to create
database and tables on your MySQL server as described in
F<contrib/write_mysql.sql>. You can do this using:

 mysql -u "${USER}" -p "${DATABASE}" <contrib/write_mysql.sql
